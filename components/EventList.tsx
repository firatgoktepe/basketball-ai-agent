...
import { Trophy, Target, RotateCcw, AlertTriangle, Edit, Check, X } from 'lucide-react'
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`px-3 py-1 rounded text-sm ${
              filter === 'high_confidence' ? 'bg-primary text-primary-foreground' : 'bg-muted'
            }`}
          >
            High Confidence
          </button>
          <button
            ...
            className={`px-3 py-1 rounded text-sm ${
              filter === 'low_confidence' ? 'bg-primary text-primary-foreground' : 'bg-muted'
            }`}
          >
            Low Confidence
          </button>
              ...
              {isEditing && (
                <div className="mt-4 p-3 bg-white border rounded">
                  <div className="text-sm text-muted-foreground mb-2">
                    Event editing functionality will be implemented in future updates.
                  </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            ...
            <div className="font-semibold">
              {Math.round(
                gameData.events.reduce((sum, e) => sum + e.confidence, 0) / gameData.events.length * 100
              )}%
            </div>
          </div>
        </div>

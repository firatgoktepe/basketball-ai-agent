...
export function extractJerseyColors(detections: DetectionResult[], frames: ImageData[]): ColorSample[] {
    ...
    for (const result of detections) {
        ...
        for (const detection of result.detections) {
            ...
            colorSamples.push(...colors.map(color => ({
                ...color,
...
function extractColorsFromBbox(frame: ImageData, bbox: [number, number, number, number]): Array<{ r: number, g: number, b: number, h: number, s: number, v: number }> {
    const [x, y, width, height] = bbox
    const colors: Array<{ r: number, g: number, b: number, h: number, s: number, v: number }> = []
...
function isJerseyColor(r: number, g: number, b: number, h: number, s: number, v: number): boolean {
...
export function clusterColorsByKMeans(colorSamples: ColorSample[], k: number = 2): Array<{ centroid: { r: number, g: number, b: number }, samples: ColorSample[], teamId?: string }> {
    if (colorSamples.length < k) {
        return []
    }

    const clusters: Array<{ centroid: { r: number, g: number, b: number }, samples: ColorSample[], teamId?: string }> = []

    // Initialize centroids with random samples
    for (let i = 0; i < k; i++) {
        const randomSample = colorSamples[Math.floor(Math.random() * colorSamples.length)]
        clusters.push({
    ...
    for (let iteration = 0; iteration < 10; iteration++) {
        ...
        for (const cluster of clusters) {
            if (cluster.samples.length > 0) {
                cluster.centroid.r = cluster.samples.reduce((sum, s) => sum + s.r, 0) / cluster.samples.length
                cluster.centroid.g = cluster.samples.reduce((sum, s) => sum + s.g, 0) / cluster.samples.length
                cluster.centroid.b = cluster.samples.reduce((sum, s) => sum + s.b, 0) / cluster.samples.length
            }
        }
    }
...
function assignTeamIds(clusters: Array<{ centroid: { r: number, g: number, b: number }, samples: ColorSample[], teamId?: string }>): void {
...
export function assignTeamToDetection(detection: any, colorSamples: ColorSample[], clusters: Array<{ centroid: { r: number, g: number, b: number }, samples: ColorSample[], teamId?: string }>): string | undefined {

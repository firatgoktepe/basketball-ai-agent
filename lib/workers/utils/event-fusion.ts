...
export async function fuseEvents(options: FusionOptions): Promise<GameEvent[]> {
  const { personDetections, ballDetections, poseDetections, ocrResults, enable3ptEstimation } = options
  ...
  const reboundEvents = detectRebounds(shotEvents, ballDetections, personDetections)
  ...
  if (enable3ptEstimation) {
    const threePointEvents = detectThreePointAttempts(poseDetections, shotEvents)
...
function detectShotAttempts(poseDetections: any[], ballDetections: any[]): GameEvent[] {
  const events: GameEvent[] = []
  ...
  for (const poseResult of poseDetections) {
    for (const pose of poseResult.poses) {
      ...
      if (isShooting) {
        ...
        const nearbyBall = findNearbyBall(poseResult.timestamp, ballDetections, pose.bbox)
        ...
        events.push({
          ...
          notes: nearbyBall ? 'Detected by pose and ball tracking' : 'Detected by pose analysis'
        })
      }
    }
  }
...
function detectShootingPose(keypoints: Array<{ x: number; y: number; confidence: number }>): boolean {
...
function findNearbyBall(timestamp: number, ballDetections: any[], bbox: [number, number, number, number]): boolean {
...
function detectRebounds(shotEvents: GameEvent[], ballDetections: any[], personDetections: any[]): GameEvent[] {
  const events: GameEvent[] = []
  ...
  for (const shot of shotEvents) {
    ...
    const reboundEvents = findReboundEvents(shot, ballDetections, personDetections, reboundWindow)
    events.push(...reboundEvents)
  }
...
function findReboundEvents(shot: GameEvent, ballDetections: any[], personDetections: any[], window: number): GameEvent[] {
  const events: GameEvent[] = []
  const endTime = shot.timestamp + window
  
  // Look for ball and player interactions in the rebound window
  for (const ballResult of ballDetections) {
    if (ballResult.timestamp > shot.timestamp && ballResult.timestamp <= endTime) {
...
function detectTurnovers(personDetections: any[], ballDetections: any[]): GameEvent[] {
  const events: GameEvent[] = []
  ...
  // Mock implementation
  if (Math.random() > 0.9) { // 10% chance of turnover
    events.push({
...
function detectThreePointAttempts(poseDetections: any[], shotEvents: GameEvent[]): GameEvent[] {
  const events: GameEvent[] = []
  ...
  for (const shot of shotEvents) {
    if (Math.random() > 0.7) { // 30% chance of 3PT attempt
      events.push({

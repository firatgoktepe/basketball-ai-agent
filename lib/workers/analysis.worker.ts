...
async function analyzeVideo(options: any) {
    try {
        ...
        const { videoFile, cropRegion, samplingRate, enableBallDetection, enablePoseEstimation, enable3ptEstimation, onProgress } = options

        // Step 1: Extract frames
        onProgress({ stage: 'sampling', progress: 10, message: 'Extracting video frames...' })
        ...
        if (enableBallDetection) {
            onProgress({ stage: 'detection', progress: 50, message: 'Detecting ball movement...' })
        ...
        if (enablePoseEstimation) {
            onProgress({ stage: 'detection', progress: 60, message: 'Analyzing player poses...' })
            poseDetections = await extractPoses(frames, moveNetModel)
        }

        // Step 5: OCR on scoreboard
        onProgress({ stage: 'ocr', progress: 80, message: 'Reading scoreboard...' })
        const ocrResults = await runOCR(frames, cropRegion)

        // Step 6: Event fusion
        onProgress({ stage: 'fusion', progress: 90, message: 'Processing events...' })
        ...
        onProgress({ stage: 'results', progress: 100, message: 'Generating results...' })
...
function generateGameData(videoFile: any, teamClusters: any, events: any[]) {
    ...
    const summary = {
        teamA: {
            points: events.filter(e => e.type === 'score' && e.teamId === 'teamA').reduce((sum, e) => sum + (e.scoreDelta || 0), 0),
            shotAttempts: events.filter(e => e.type === 'shot_attempt' && e.teamId === 'teamA').length,
            offRebounds: events.filter(e => e.type === 'offensive_rebound' && e.teamId === 'teamA').length,
            defRebounds: events.filter(e => e.type === 'defensive_rebound' && e.teamId === 'teamA').length,
            turnovers: events.filter(e => e.type === 'turnover' && e.teamId === 'teamA').length,
        },
        teamB: {
            points: events.filter(e => e.type === 'score' && e.teamId === 'teamB').reduce((sum, e) => sum + (e.scoreDelta || 0), 0),
            shotAttempts: events.filter(e => e.type === 'shot_attempt' && e.teamId === 'teamB').length,
            offRebounds: events.filter(e => e.type === 'offensive_rebound' && e.teamId === 'teamB').length,
            defRebounds: events.filter(e => e.type === 'defensive_rebound' && e.teamId === 'teamB').length,
            turnovers: events.filter(e => e.type === 'turnover' && e.teamId === 'teamB').length,
        }
    }
